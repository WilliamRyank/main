...
After adding a person, OTM will display a form which contains the attributes of the person. Press
...
OTM will prompt the user to type in each attribute, one by one.

In the display box, there will be a form that reflects the attributes typed by the user.

After the inputs are added, OTM will display a template that shows what the user data will look like
...
Currently, OTM only supports kidney matching. Refer to <<FAQ>> to know the
...
update the results in OTM. Cross matching tests have two possible results: pass or fail.
...
* *Delete* : `delete t/**TYPE** ic/**NRIC**` +
* *Edit* : `edit t/**TYPE** ic/**NRIC**`
** Optional parameters: any parameter from <<List of Attributes>>
* *Find* : `find`
** Optional parameters: any parameter from <<List of Attributes>>
* *Match* : `match [all/ic]`
